...
def _sample_params(choices_dict: dict, base_params: dict) -> dict:
    """choices_dict에서 랜덤 샘플 1개 뽑아 base_params에 덮어쓰기"""
    p = dict(base_params)
    for k, arr in (choices_dict or {}).items():
        if not arr: 
            continue
        v = random.choice(arr)
        if v == "same":      # ma_compare_long = 'same' 같은 케이스
...
def auto_search_train_test(
    *, signal_ticker, trade_ticker, start_date, end_date,
    ...
    """간단한 Train/Test 랜덤 탐색 (objective_metric: '수익률 (%)' 등)"""
    # 데이터 로딩 1회
    # train/test를 날짜로 나눠 prepare_base 두 번 호출
    s = pd.to_datetime(start_date); e = pd.to_datetime(end_date)
    cutoff = s + (e - s) * float(split_ratio)

    # 후보군에서 필요한 MA window 모으기
    ...
    execution_price_mode=execution_price_mode
        ...
        if res_te.get("총 매매 횟수", 0) < int(min_trades):
            continue
        if res_te.get("승률 (%)", 0.0) < float(min_winrate):
            continue
        if (max_mdd is not None) and (res_te.get("MDD (%)", 0.0) < -abs(max_mdd)):  # MDD는 음수
            continue

        row = {
            # Train
            "TR 수익률 (%)": res_tr.get("수익률 (%)"),
            "TR 승률 (%)": res_tr.get("승률 (%)"),
            "TR MDD (%)": res_tr.get("MDD (%)"),
            "TR Profit Factor": res_tr.get("Profit Factor"),
            "TR 총 매매 횟수": res_tr.get("총 매매 횟수"),
            # Test
            "수익률 (%)": res_te.get("수익률 (%)"),
            "승률 (%)": res_te.get("승률 (%)"),
            "MDD (%)": res_te.get("MDD (%)"),
            "Profit Factor": res_te.get("Profit Factor"),
            "총 매매 횟수": res_te.get("총 매매 횟수"),
            # Params
            "ma_buy": p["ma_buy"], "offset_ma_buy": p["offset_ma_buy"], "offset_cl_buy": p["offset_cl_buy"], "buy_operator": p["buy_operator"],
            "ma_sell": p["ma_sell"], "offset_ma_sell": p["offset_ma_sell"], "offset_cl_sell": p["offset_cl_sell"], "sell_operator": p["sell_operator"],
            "use_trend_in_buy": p["use_trend_in_buy"], "use_trend_in_sell": p["use_trend_in_sell"],
...
with tab3:
    ...
    # 실행 버튼
    if st.button("✅ 백테스트 실행", use_container_width=True, key="btn_bt_run"):
        # 1) MA 풀 구성
        ...
        if result:
            ...
            if not df_log.empty and "날짜" in df_log.columns:
                ...
                # 벤치마크 (Buy&Hold)
                if "종가" in df_log.columns and not df_log["종가"].isna().all():
                ...
                # 종가 (오른쪽 y축)
                if "종가" in df_log.columns:
                    fig.add_trace(go.Scatter(
                        x=df_log.index, y=df_log["종가"], mode="lines", name="Price", yaxis="y2"
                    ))
                    # ✅ 20MA 추가
                    df_log["MA20"] = df_log["종가"].rolling(20).mean()
                    fig.add_trace(go.Scatter(
                        x=df_log.index, y=df_log["MA20"], mode="lines", name="20일 MA", yaxis="y2",
                        line=dict(dash="dash")
                    ))

                # 보유 구간 음영
                if "신호" in df_log.columns:
                    ...
                    fig.add_trace(go.Scatter(
                        x=buy_points.index, y=buy_points.get("종가"), mode="markers", name="BUY", yaxis="y2",
                        marker=dict(symbol="triangle-up", size=6)
                    ))
                    fig.add_trace(go.Scatter(
                        x=sell_points.index, y=sell_points.get("종가"), mode="markers", name="SELL", yaxis="y2",
                        marker=dict(symbol="triangle-down", size=6)
                    ))

                    # 동시 만족(양시그널) 보조 — 없을 수도 있으니 안전 처리
                    if "양시그널" in df_log.columns:
                        both_buy = buy_points[buy_points["양시그널"] == True]
                        both_sell = sell_points[sell_points["양시그널"] == True]
                        fig.add_trace(go.Scatter(
                            x=both_buy.index, y=both_buy.get("종가"), mode="markers", name="BUY (양시그널)", yaxis="y2",
                            marker=dict(symbol="triangle-up", size=9, line=dict(width=2))
                        ))
                        fig.add_trace(go.Scatter(
                            x=both_sell.index, y=both_sell.get("종가"), mode="markers", name="SELL (양시그널)", yaxis="y2",
                ...
                st.download_button("⬇️ 백테스트 결과 다운로드 (CSV)", data=csv,
