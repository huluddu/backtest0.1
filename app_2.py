...
with tab3:
    ...
    if st.button("✅ 백테스트 실행", use_container_width=True):
        ...
        if result:
            ...
            fig.add_trace(go.Scatter(
                x=df_log.index, y=pos_asset, mode="lines", name="In-Position",
            ...
            fig.add_trace(go.Scatter(
                x=buy_points.index, y=buy_points["종가"], mode="markers", name="BUY",
                yaxis="y2", marker=dict(color="green", size=6, symbol="triangle-up")
            ))
            fig.add_trace(go.Scatter(
                x=sell_points.index, y=sell_points["종가"], mode="markers", name="SELL",
                yaxis="y2", marker=dict(color="red", size=6, symbol="triangle-down")
            ))
            fig.add_trace(go.Scatter(
                x=both_buy.index, y=both_buy["종가"], mode="markers", name="BUY (양시그널)",
                yaxis="y2", marker=dict(color="green", size=9, symbol="triangle-up", line=dict(color="yellow", width=2))
            ))
            fig.add_trace(go.Scatter(
                x=both_sell.index, y=both_sell["종가"], mode="markers", name="SELL (양시그널)",
                yaxis="y2", marker=dict(color="red", size=9, symbol="triangle-down", line=dict(color="yellow", width=2))
            ))
            ...
            for _, r in df_log.reset_index().iterrows():
                if r["신호"] == "BUY":
                    ...
                    pb = buy_cache.get("체결가") if pd.notna(buy_cache.get("체결가")) else buy_cache["종가"]
                    ps = r.get("체결가") if pd.notna(r.get("체결가")) else r["종가"]
                    pnl = (ps - pb) / pb * 100
                    pairs.append({
                        "진입일": buy_cache["날짜"],
                        "청산일": r["날짜"],
            ...
            # 결과/로그 다운로드
            with st.expander("🧾 매매 로그", expanded=False):
                st.dataframe(df_log)
